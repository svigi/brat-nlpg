...
var AnnotatorUI = (function($, window, undefined) {
	...
	$('#testbutton').click(function() {
		...
		console.log('ezt küldi majd:')
		console.log(entityData)
		var valaszfv = function(response) {
			console.log('Válasz:');
			console.log(response);
			alert(response['test']);
		}
		dispatcher.post('ajax', [entityData, 'valaszfv']);
	...
	var newfv = function() {
		dispatcher.post('ajax', [{
			...
			docname : 'szemeszet',
			value : $('#testbutton').val(),
		},
		function(response) {
			console.log('Válasz:');
			console.log(response);
			var szoveg = $.dump(response);
			$("entity_and_event_wrapper_fieldset").css("display", "none");
			$("entity_and_event_wrapper_fieldset").hide("fast");
			alert(response['test']);
			$("entity_and_event_wrapper_fieldset").css("display", "none");
			$("entity_and_event_wrapper_fieldset").hide("fast");
			console.log('teszt');
	...
	var AnnotatorUI = function(dispatcher, svg) {
		...
		var onKeyDown = function(evt) {
			...
			if (!binding)
				binding = keymap[String.fromCharCode(code)];
			if (binding) {
		...
		var onDblClick = function(evt) {
			...
			if (reselectedSpan || arcDragOrigin)
				return;
			...
			if ( id = target.attr('data-arc-role')) {
				...
				// TODO fill from info to be provided by server
				...
				editPosStart = editedSpan.from;
				editPosEnd = editedSpan.to;

				//alert('fromedit: ' + editedSpan.from + ', toedit: ' + editedSpan.to);
				//var $entityScroller = $('#entity_types div.scroller');
				fillSpanTypesAndDisplayForm(evt, spanText, editedSpan);
				addUserDefinedSpanTypesToDivInner(editedSpan.from, editedSpan.to, editedSpan, spanText);
		...
		var getValidArcTypesForDrag = function(targetId, targetType) {
			if (!arcDragOrigin || targetId == arcDragOrigin)
				return null;
		...
		var onMouseMove = function(evt) {
			if (arcDragOrigin) {
				...
				var my = evt.pageY - svgPosition.top + 5;
				// TODO FIXME why +5?!?
		...
		var adjustToCursor = function(evt, element, centerX, centerY) {
			var screenHeight = $(window).height() - 8;
			// TODO HACK - no idea why -8 is needed
		...
		var fillSpanTypesAndDisplayForm = function(evt, spanText, span) {
			...
			//console.log(span)
			//console.log(resp)
			//console.log('loggvége')
			$("entity_and_event_wrapper_fieldset").css("display", "block");

			$("#otherentity_container").css("display", "none");
			$('#span_otherentity').val('');
			$("#otherentity_toggler").removeAttr('checked');

			$('#span_entity_section input:radio').click(function() {

				//alert('katt');
				if ($("#otherentity_toggler").is(":checked")) {
					$("#otherentity_toggler").removeAttr('checked');
					$("#otherentity_container").hide("fast");
				}
			...
			if (span) {
				...
				// else {
				// // alert('Nincs ilyen entity név!');
				// // ekkor az összes radio button tiltva van, ekkor kell az Other engedélyezése!
				//
				// //alert("szöveg beírása");
				// //beírjuk a szöveget
				// $("#span_otherentity").val(span.type);
				// $("#otherentity_container").show("fast");
				// var toggler = $("#otherentity_toggler");
				// if (toggler.length) {
			...
			var showAttributesFor = function(attrTypes, type) {
				...
				//workaround for the new external entities
				var validAttrs;
				if (type)
					if (spanTypes[type] != undefined)
						if (spanTypes[type].attributes != undefined) {
							validAttrs = spanTypes[type].attributes;
						} else
							validAttrs = [];

				//old solution
				//validAttrs = type ? spanTypes[type].attributes : [];
				var shownCount = 0;
		...
		var rapidFillSpanTypesAndDisplayForm = function(start, end, text, types) {
			...
			$spanTypeDiv.append($('<div class="item_content">&#160;</div>'));
			// non-breaking space
		...
		var fillArcTypesAndDisplayForm = function(evt, originType, targetType, arcType, arcId, arcAnnotatorNotes) {
			...
			var isEquiv = relationTypesHash && relationTypesHash[noNumArcType] && relationTypesHash[noNumArcType].properties.symmetric && relationTypesHash[noNumArcType].properties.transitive;
			...
			if (spanTypes[originType]) {
				...
				$.each(arcTypes || [], function(arcTypeNo, arcDesc) {
					if (arcDesc.targets && arcDesc.targets.indexOf(targetType) != -1) {
						...
						var isThisEquiv = relationTypesHash && relationTypesHash[arcTypeName] && relationTypesHash[arcTypeName].properties.symmetric && relationTypesHash[arcTypeName].properties.transitive;

						// do not allow equiv<->non-equiv change options
						if (arcType && isEquiv != isThisEquiv)
							return;
			...
			if (arcId) {
				...
				$('#arc_highlight_link').attr('href', document.location + '/' + arcId).show();
				// TODO incorrect
			...
			if (arcDragOrigin) {
				...
				if (chunkIndexFrom !== undefined && chunkIndexTo !== undefined) {
					...
					if (crossSentence) {
						...
						//selectedfromto!!
						//alert('from: ' + selectedFrom + ', to: ' + selectedTo);

						editPosStart = selectedFrom;
						editPosEnd = selectedTo;
						fillSpanTypesAndDisplayForm(evt, spanText, reselectedSpan);
						addUserDefinedSpanTypesToDivInner(selectedFrom, selectedTo, null, spanText);
						//alert('fill után')
		...
		var addUserDefinedSpanTypesToDivInner = function(fromPos, toPos, span, text) {

			var $dest = $('#testcontainer2');
			$dest.html('Loading...');

			var types = new Array();
			//console.log('docneve ' + doc)
			dispatcher.post('ajax', [{
				action : 'getEntities',
				start : fromPos,
				end : toPos,
			...
			function(response) {
				...
				if (!response) {
					alert('Error!')
					//$("entity_and_event_wrapper").show("fast");
					return;
				}
				...
				if (response['entities'].length == 0)
					response['entities'].push(text);

				$dest.html('');
				//$("entity_and_event_wrapper").hide("fast");
				$.each(response['entities'], function(index, value) {
					//stuff = '';
					//for(i = 0; i < value.length; i++) {
					//stuff += value[i] + ';';
					...
					var $input = $('<input type="radio" name="span_type"/>').attr('id', 'span_' + entityName).attr('value', entityName);

					var $label = $('<label class="span_type_label"/>').attr('for', 'span_' + entityName).text(entityName);
				...
				$('#testcontainer2 input:radio').click(function() {
					//alert('katt');
					if ($("#otherentity_toggler").is(":checked")) {
						$("#otherentity_toggler").removeAttr('checked');
						$("#otherentity_container").hide("fast");
				...
				checkedBoxes = 0
				$('#testcontainer2 input:radio:checked').each(function(radioNo, radio) {
					checkedBoxes++;
				});
		...
		var spanAndAttributeTypesLoaded = function(_spanTypes, _entityAttributeTypes, _eventAttributeTypes, _relationTypesHash) {
			...
			$("#otherentity_toggler").click(function() {
		...
		var spanFormSubmit = function(evt, typeRadio) {
			...
			//TODO: állítsa össze a listát
			var lista = new Array();

			//új, még nem létezőt írt be
			//ekkor az új bejegyzés a lista elejére kerül, a többi meg marad sorrendben

			if ($("#otherentity_toggler").is(":checked")) {
				//alert("other checked");
				var otherentityvalueval = $('#span_otherentity').val();

				//szóköz és kettőspont nem megengedettek (bezavar)
				var otherentityvalue = otherentityvalueval.replace(/ /g, '_');
				var otherentityvalue = otherentityvalue.replace(/\:/g, '_');
				//alert(otherentityvalue);
				//return false;
				//alert("val = " + otherentityvalue);
				if (otherentityvalue != '') {
					//alert("nem üres");
					//var bekeres = prompt("Kérem a szöveget", otherentityvalue);
					//alert ( "A bevitt név: " + bekeres);
					$.extend(spanOptions, {
						type : otherentityvalue
					});

					lista.push(otherentityvalue);

					$('#testcontainer2 input:radio').each(function(index) {
						//alert(index + ': ' + $(this).val());
						lista.push($(this).val());
					});

					//alert("most1");
					//$('#span_form input:radio')[0].checked = true;
					//alert("most2");
				} else {
					alert("Please fill in the word form!");
					return false;
				}
			} else {
				//TODO: meglévőből választott a kiválasztott lesz az első, a többi változatlan sorrendben utána következik
				//alert("other not checked");
				var checkedBoxNo = -1;

				var checkedBoxes = 0;
				$('#span_form input:radio:checked').each(function(radioNo, radio) {
					checkedBoxes++;
					checkedBoxNo = radioNo;
				});

				//TODO: több, mint egy radiobutton kiválasztásának ellenőrzése?
				if (checkedBoxes == 0) {
					alert("Please select a word form!");
					return false;
				} else {
					$.extend(spanOptions, {
						type : type
					...
					$('#testcontainer input:radio:checked').each(function(radioNo, radio) {
						lista.push($(this).val())
					});

					$('#testcontainer input:radio:not(:checked)').each(function(radioNo, radio) {
						lista.push($(this).val())
					});
